...
>
> **Download pipeline: done.** `aspnetzero.com` branch `feature/ai-config-layout-15-5` (not pushed) makes `ProjectUiTemplateFilter` version-aware: 15.4 and earlier keep getting the `.agent/` layout, 15.5 and later get `.agents/`, and `dev` resolves to the newest. `ProjectBuilder` passes the project version in. The per-vendor path rules are now generated from a small table instead of ~90 lines of repetition, which also fixed a rule that had already gone stale: nothing removed the ng-zorro table skill from a React or MVC download.
>
