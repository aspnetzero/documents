...
>
> **Download pipeline: done.** `aspnetzero.com` branch `feature/ai-config-layout-15-5` (not pushed) makes `ProjectUiTemplateFilter` version-aware: the layout is detected from the template content itself (a template shipping `.agents/` gets the modern rules, anything else the legacy rules), so the GitHub-outage fallback that pairs a cached template of a different version with the requested version string can never mis-filter, and deployment order relative to the template repos does not matter. The per-vendor path rules are now generated from a small table instead of ~90 lines of repetition, which also fixed a rule that had already gone stale: nothing removed the ng-zorro table skill from a React or MVC download.
>
> The generated vendor files are committed to the template, so the pipeline does **not** have to run `sync-agent-config`. Running it as a last step is still worth considering as belt and braces; it has no npm dependencies and takes about a second.
>
> Verified with a throwaway harness driving `ProjectUiTemplateFilter.Apply` over the real file lists of `origin/rel-15.4` and `dev`: 52 assertions across Angular, React and MVC in both layouts, including a guard showing that legacy rules leave a modern tree untouched.
>
> One trap found and fixed while doing this: the filter's block regex matches a marker **anywhere on a line**, including inside backticks in prose. `.agents/README.md` documented the mechanism by spelling out a begin marker and an end marker on consecutive lines, so the pipeline would have silently deleted both lines from every generated project. Shipped files must never spell a marker out in full.
...
- [x] **Implement the Health Check page in the React UI** so that all three UIs are at parity. Done on the `feature/react-health-check-page` branch of `aspnet-zero-core`: `react/src/pages/admin/health-check/index.tsx` plus the navigation entry and route, mirroring the Angular page (10-second auto-refresh, manual refresh, status tags, duration in ms).
