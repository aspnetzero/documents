...
> Note: Only providers that are enabled in `appsettings.json` and active for the current tenant are shown. See [Social & External Logins](Features-React-Social-Logins) for how to configure providers.
...
> Note: In the React application, all providers (Facebook, Google, Microsoft, OpenID Connect, WsFederation, and Twitter) use a **client-side popup flow** for authentication. You authenticate in a popup window and are redirected back automatically.

### Authentication Flow by Provider

The React application uses two different authentication flows depending on the provider:
