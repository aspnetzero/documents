...
ASP.NET Zero uses [React toolchain](https://cli.React.io/) to build the React application and it creates style and script bundles. But for some cases, an React app might decide which style/script file to use at runtime and loads this style/script file dynamically. For such cases, ASP.NET Zero provides a bundling and minification system.
...
For production usage, you can run "**npm run publish**" and it will minify dynamic assets for you and then it will publish your React app.
...
> Note: Don't use the bundle.json unless you have a dynamic css/js file. Normally, React toolchain manages your css and js file.
