...
The External Logins tab is located in the **My Settings** modal. To open it, click on your profile picture or username in the top bar and select **My Settings**. Then switch to the **External Logins** tab.
...
> Note: Only providers that are enabled in `appsettings.json` and active for the current tenant are shown. See [Social & External Logins](Features-Angular-Social-Logins) for how to configure providers.
...
3. A popup window will appear asking you to authenticate with the selected provider.
4. After successful authentication, the provider is linked to your account and the status changes to **Linked**.

![Linking a social account - popup authentication](images/external-logins-link-popup.png)
...
> Note: In the Angular application, all providers (Facebook, Google, Microsoft, OpenID Connect, WsFederation, and Twitter) use a **client-side popup flow** for authentication. You authenticate in a popup window and are redirected back automatically.

### Authentication Flow by Provider

The Angular application uses two different authentication flows depending on the provider:
