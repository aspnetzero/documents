...
**Note:** The RAD Tool automatically generates Mapperly mapper classes for entity to DTO mappings. Unlike the previous AutoMapper approach that used a centralized `CustomDtoMapper.cs` file, Mapperly uses individual mapper classes that are created per entity.
