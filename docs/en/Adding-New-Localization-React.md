...
The localization XML file contains key-value pairs for translating text within the application. Each language has its own XML file where you define translations for UI elements, messages, and labels. This file ensures that the application can display content in the selected language. Create a new localization file next to the existing ones, in the **Localization/YourProjectName** folder of the `YourProjectName.Core` project.
...
Localized texts themselves are served to the React application by the API, so no text has to be duplicated on the client side.

### 2. Add Language to React

The React UI uses [Ant Design](https://ant.design/) components (date pickers, tables, pagination and so on) and [Day.js](https://day.js.org/) for date formatting. Both carry their own localized texts and need to know about the new language.

#### Register the Ant Design Locale

Ant Design locales are imported on demand in `src/lib/andt-locale-loader.ts`. Only the locales listed in the map are included in the application bundle, so add an entry for the new language:
...
export async function loadLocale(lang: string): Promise<Locale> {
    ...
    const antdLocaleMap: Record<string, () => Promise<{ default: Locale }>> = {
        ...
        pl: () => import('antd/locale/pl_PL'), // Add the new language here
        // Other locales
    };
...
The key is the ABP language name with the dash removed (`pt-BR` becomes `ptBR`), and the value is the matching locale of the `antd/locale` entry point. `useAntdLocale` resolves the current ABP language through this map and hands the result to Ant Design's `ConfigProvider`. If there is no entry for the current language, `en_US` is used as the fallback.

#### Day.js Locale

Day.js locales are loaded dynamically by `configureDayjsLocale` in `src/lib/abp-base.tsx`, using the ABP language name directly:
...
No registration is needed as long as Day.js ships a locale file with that name (`dayjs/locale/pl` for the example above). If it does not, the application silently falls back to English.
...
- **Restart the Application**
    - Restart both the backend and the frontend to ensure that all localization changes take effect.
...
By following these steps, you have successfully added support for a new language in your ASP.NET Zero React application. The localization process ensures that both backend and frontend components, including UI elements and date pickers, properly reflect the new language.
